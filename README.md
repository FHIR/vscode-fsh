...
The extension provides a second custom task that runs the FHIR validator on the resources that have been generated by SUSHI. The task validate all files in `fsh-generated/resources`, log messages to VS Code's integrated Terminal tab, report any errors or warnings in VS Code's Problems tab, and highlight errors and warnings inline in the .JSON file. Selecting an error or warning from the Problems tab will open the file the error is in.   Tracing the problem in the generated JSON file back to the orginal .FSH file is left as an exercise for the user.
